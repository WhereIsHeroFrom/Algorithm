...
const unsigned int MAXDIGITS = 102;        // 大数的位数，即 10^MAXDIGITS
...
/*
	dp[i][j] 代表以第 i 个元素结尾，长度为 j 的递增子序列的个数
	
	dp[i][j] = sum{ dp[s][j-1]  | a[s] < a[i] }
	( 1 <= i,j <= n )
	
	dp[0][0] = 1;
*/

BigNum dp[110][110], ans;
int a[110];
