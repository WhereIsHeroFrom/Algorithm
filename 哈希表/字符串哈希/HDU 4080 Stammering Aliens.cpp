...
const int BASE[] = { 12289, 133, 233 };
const int MOD[] = { 19260817, 1e9 + 7, 1e9 + 9 };
...
bool checkOccurTimes(int times, int L, int maxLength, int& lastpos) {
    memset(cnt, 0, sizeof(cnt));
    h.initialize();
    bool bflag = false;
    for (int i = 1; i + L - 1 <= maxLength; ++i) {
        HashType stringHash = sh.get(i, i + L - 1);
        int haskey = h.setKey(stringHash);
        if (++cnt[haskey] >= times) {
            bflag = true;
            lastpos = i - 1;
        }
...
int main() {
    ...
    while (scanf("%d", &m) != EOF && m) {
        scanf("%s", &str[1]);
        //for (int i = 0; i < 40000; ++i) str[i] = 'a';
        //str[40000] = 0;
        int maxLen = strlen(&str[1]);
        sh.initialize(maxLen, str);
        ...
        while (l <= r) {
            int mid = (l + r) >> 1;
            int lastpos = 0;
            if (checkOccurTimes(m, mid, maxLen, lastpos)) {
                ans = mid;
                las = lastpos;
                l = mid + 1;
            }
