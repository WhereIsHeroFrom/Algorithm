...
void ConstructMatrix(LL AX, LL AY, LL BX, LL BY, LL A0, LL B0) {
	LL AXBX = AX * BX % MOD;
	LL AXBY = AX * BY % MOD;
	LL AYBX = AY * BX % MOD;
	LL AYBY = AY * BY % MOD;

	AArray[0][0] = 1;   AArray[0][1] = AXBX;     AArray[0][2] = AXBY;    AArray[0][3] = AYBX;    AArray[0][4] = AYBY;
	AArray[1][0] = 0;   AArray[1][1] = AXBX;     AArray[1][2] = AXBY;    AArray[1][3] = AYBX;    AArray[1][4] = AYBY;
	AArray[2][0] = 0;   AArray[2][1] = 0;        AArray[2][2] = AX;      AArray[2][3] = 0;       AArray[2][4] = AY;
	AArray[3][0] = 0;   AArray[3][1] = 0;        AArray[3][2] = 0;       AArray[3][3] = BX;      AArray[3][4] = BY;
	AArray[4][0] = 0;   AArray[4][1] = 0;        AArray[4][2] = 0;       AArray[4][3] = 0;       AArray[4][4] = 1;


	BArray[0][0] = A0 * B0 % MOD;
	BArray[1][0] = A0 * B0 % MOD;
	BArray[2][0] = A0 % MOD;
	BArray[3][0] = B0 % MOD;
	BArray[4][0] = 1;
...
int main() {
	...
	while (scanf("%I64d", &n) != EOF) {
		...
		ConstructMatrix(AX, AY, BX, BY, A0, B0);
		...
		if (n == 0) {
			...
			ans = ((ret2.get(0, 0) - ret2.get(1, 0)) % MOD + MOD) % MOD;
		}
