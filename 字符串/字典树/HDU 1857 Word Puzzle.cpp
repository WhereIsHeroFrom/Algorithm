...
const int TRIE_WORD_LENGTH = 20;             // 修改点 4
...
public:
    ...
    bool query_word(int vSize, ValueType v[], int pos, int *out);                // 询问是否存在单词 v
...
bool TrieTree::query_word(int vSize, ValueType v[], int pos, int *out) {
    // 查找对应字典中，是否有字符串等于 v[]
    TrieNode *pkNow = root();
    for (int i = 0; i < vSize; ++i) {
        int nodeIdx = trieNodeValueHash(v[i]);
        ...
        pkNow = node(pkNow->getNode(nodeIdx));
        if (pkNow->getTrieData() != TRIE_DATA_INIT) {
            if (out[pkNow->getTrieData()] == TRIE_DATA_INIT)
                out[pkNow->getTrieData()] = pos;
        }
    }
    return pkNow->isWord();
}
...
int main() {
    ...
    while (scanf("%s", ans[total]) != EOF)
    {
        ...
        tt.insert_word(strlen(ans[total]), ans[total], total);
    ...
    for (int i = 0; i < R; ++i) {
        for (int j = 0; j < C; ++j) {
            for (int k = 0; k < 3; ++k) {
                ...
                str[s] = '\0';
                tt.query_word(s, str, 512 * i + j, out);
            }
        }
    }

    for (int i = 0; i < total; ++i) {
        if (out[i] == TRIE_DATA_INIT) printf("-1 -1\n");
        else {
            printf("%d %d\n", out[i] / 512, out[i] % 512);
        }
    }
