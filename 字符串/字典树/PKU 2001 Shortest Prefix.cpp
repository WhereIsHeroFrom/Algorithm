/*
    字符串的最短前缀表示：
        ...
        如果某个字符串在字典树中结点计数 = 1，说明以它为前缀的字符串只有这个前缀了
    则可以用它来表示这个字符串。
*/
...
const int TRIE_WORD_LENGTH = 25;             // 修改点 4
...
int main() {
    ...
    int nameCnt = 0;
    int testCase = -13;
    while (scanf("%s", str) != EOF && --testCase) {
        tt.insert_word(strlen(str), str, 1);
        strcpy(names[nameCnt++], str);
    }

    char ans[100];
    for (int i = 0; i < nameCnt; ++i) {
        tt.query_prefix(strlen(names[i]), names[i], ans);
        printf("%s %s\n", names[i], ans);
    }
