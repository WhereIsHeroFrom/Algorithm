/*
    字典树 - 叶子结点打标记
           ...
           - 查询的时候：
                    ...
                    如果在内部结点发现 word 标记，说明它是那个字符串的前缀，返回 false；
...
const int TRIE_NODE_COUNT = 2;              // 修改点 5
...
int main() {
    ...
    while (scanf("%s", codes[c]) != EOF) {
        ...
        if (codes[c][0] == '9') {
            ...
            for (i = 0; i < c; ++i) {
                if (!tt.query_word(strlen(codes[i]), codes[i])) {
                    break;
                }
            }
            ...
            tt.insert_word(strlen(codes[c]), codes[c], 1);
            ++c;
        }
    }
