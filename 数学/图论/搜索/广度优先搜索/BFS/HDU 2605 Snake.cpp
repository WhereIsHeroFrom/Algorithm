...
public:
    ...
    bool isValidState(int row, int col);    // 1. 格子是否合法，一般是判断 阻碍 和边界
    bool isFinalState();                    // 2. 是否结束状态
    int getStep() const;                    // 3. 获取从 初始状态 到 当前状态的步数
    void setStep(int step);                 // 4. 设置从 初始状态 到 当前状态的步数
    void print();                           // 5. 调试打印当前状态用
    bool operator==(const BFSState & o);    // 6. 用来判断两个状态是否相等
