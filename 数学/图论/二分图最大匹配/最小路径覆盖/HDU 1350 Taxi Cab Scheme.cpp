...
struct P {
    ...
    void calcMin(int a, int b, int c, int d) {
        a_ = a;
        b_ = b;
        c_ = c;
        d_ = d;
    ...
    void calcTime(char *ch) {
        int a, b;
        sscanf(ch, "%d:%d", &a, &b);
        time = a * 60 + b;
    }

    bool canReach(const P& p) {
        return time + min + 1 + (ABS(c_ - p.a_) + ABS(d_ - p.b_)) <= p.time;
