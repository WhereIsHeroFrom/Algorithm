...
TreeNode* AVLRemoveRoot(TreeNode* T) {
    ...
    TreeNode* retNode;
    if (T == NULL) {
        return NULL;                    // (1) 空树无法进行删除；
    }
    if (!T->left && !T->right) {        // (2) 叶子结点，可直接删除，删除后返回空树； 
        free(T);
        return NULL;
    }
    else if (T->left && !T->right) {   // (3) 只有左子树的情况：删除根，返回左子树； 
        retNode = T->left;
        free(T);
        return retNode;
    }
    else if (!T->left && T->right) {   // (4) 只有右子树的情况：删除根，返回右子树； 
        retNode = T->right;
        free(T);
        return retNode;
    }

    if (AVLGetHeight(T->left) > AVLGetHeight(T->right)) {  // (5) 左子树更重，则从左子树选择最大值替换根结点，并且递归删除左子树对应结点；
        T->data = AVLGetMax(T->left)->data;
        T->left = AVLRemove(T->left, T->data);
    }else {                                                // (6) 右子树更重，则从右子树选择最小值替换根结点，并且递归删除右子树对应结点；         
