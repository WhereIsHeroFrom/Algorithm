...
public:
	...
	bool findLoop(int target, int start, map<int, int>& mapEdge, int *hashEdge);
};
...
int hashVertex[MAXN], vertexCases;
int pre[MAXN];

bool Graph::findLoop(int target, int start, map<int, int>& mapEdge, int *hashEdge) {
	vertexCases++;
	hashVertex[start] = vertexCases;
	pre[start] = -1;
	queue <int> Q;
	Q.push(start);
	...
	while (!Q.empty()) {
		...
		if (u == target) {
			//printf("Circle:\n");
			while (u != start) {
				int edgeIdx = mapEdge[pre[u] * 20000 + u];
				//printf("(%d->%d)\n", pre[u], u);
				hashEdge[edgeIdx] = 1;
				u = pre[u];
			}
		...
		for (Edge *e = getHead(u); e; e = e->next) {
			int v = e->toVertex;
			int edgeIdx = mapEdge[u * 20000 + v];
			if (hashEdge[edgeIdx]) {
				continue;
			}
			if (hashVertex[v] < vertexCases) {
				hashVertex[v] = vertexCases;
				pre[v] = u;
				Q.push(v);
			}
		}
...
bool GraphEx::findLoop(int target, int start, map<int, int>& mapEdge, int *hashEdge) {
	return orgGraph->findLoop(target, start, mapEdge, hashEdge);
}
...
bool solve() {
	...
	g.init(n, ESCCA_TARJAN);

	map<int, int> edgeMap;
	bool hasMultipleEdge = false;
	int totedge = 0;
	memset(hashEdge, 0, sizeof(hashEdge));
	int x, y;
	while (scanf("%d %d", &x, &y) != EOF && (x || y)) {
		...
		if (edgeMap.find(h) == edgeMap.end()) {
			...
			hasMultipleEdge = true;
		}
	}
	g.SAT();
	// 1.强连通分量>1，则不满足CACTUS的条件 
	if (g.sccCount > 1) {
		return false;
	}
	// 2.强连通风量==1，并且有重边a->b，因为b->a必然有路径，
	// 那么b->a这条路径上的边必然属于至少两个环(画个图) 
	if (g.sccCount == 1 && hasMultipleEdge) {
		return false;
	}
	int i;
	for (i = 0; i < totedge; ++i) {
		if (!hashEdge[i]) {
			hashEdge[i] = 1;
			if (!g.findLoop(E[i].start, E[i].end, edgeMap, hashEdge)){
				return false;
			}
		}
	}
