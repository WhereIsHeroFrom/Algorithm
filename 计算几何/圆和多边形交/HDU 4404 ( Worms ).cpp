...
int main() {
    ...
    while( scanf("%lf %lf %lf %lf %lf %lf %lf", &x0, &y0, &v0, &thi, &t, &g, &r) != EOF ) {
        if(x0<=0 && y0<=0 && v0<=0 && thi<=0 && t<=0 && g<=0 && r<=0 ) {
            break;
        }
        thi = thi/180.0*PI;

        x0 += v0*cos(thi)*t;
        y0 += v0*sin(thi)*t - g/2*t*t;
        Circle C(Point2D(x0, y0), r);
