...
double Dp[MAXN][MAXN];          // Dp[i][j] 以 第 Oi 为直线，右边围起来 j 个点的最小长度
...
int Polygon::dp(double l) {
    ...
    for (i = 0; i < n_; i++) {
        dist[i][i] = 0;
        for (j = i + 1; j < n_; ++j) {
            dist[i][j] = dist[j][i] = sqrt(0.0 + point_[i].distSquare(point_[j]));
        }
    }
    for (i = 0; i < n_; ++i) {
        for (j = 0; j <= n_; ++j) {
    ...
    for (i = 1; i < n_; ++i) {
        // i - k 之间不包含原点和k点，有多少的点在 三角形 Oik 内部
        for (k = 1; k < i; ++k) {
            int cnt = 1;    // 这个点是 i
            for (int s = k + 1; s < i; ++s) {
                BigPointType crossRet = Segment2D(point_[k], point_[s]).cross(point_[i]);
                if (TernaryFunc(crossRet) >= 0) {
                    ++cnt;
                }
            }
            ...
            for (j = cnt + 1; j <= i; ++j) {
                if (j - cnt == 1) {
                    Dp[i][j] = Min(Dp[i][j], Dp[k][j - cnt] + dist[i][k] + dist[0][i]);
                }
                else {
                    Dp[i][j] = Min(Dp[i][j], Dp[k][j - cnt] - dist[0][k] + dist[i][k] + dist[0][i]);
                }
            }
